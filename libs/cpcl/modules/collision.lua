...
function collision.packet_from_entity(entity, z_offset, dt)
	local packet = {}
	...
	packet.velocity:scale(packet.velocity, dt)
...
function collision.check_triangle(packet, triangle, cull_back_face)
	...
	if found_collison then
		...
		if not packet.found_collision or dist_to_collision < packet.nearest_distance then
			...
			local pp = packet.e_base_point:clone()
			pp.z = pp.z + packet.z_offset / packet.e_radius.z

			local n = cpml.vec3():normalize(collision_point - pp)
...
function collision.collide_with_world(packet, position, velocity, slope_threshold, depth)
	...
	local nv = (cpml.vec3():normalize(packet.velocity)):dot(slide_plane.normal)
	local fire = require "fire"
	fire.print(cpml.utils.round(nv, 0.001), 10, 4)

	-- local lift = true
	-- if not packet.lifter and new_base_point.z > new_destination_point.z then
	-- 	return position
	-- end

	-- Recurse:
	-- dont recurse if the new velocity is very small
	if new_velocity:len() < very_close_distance and lift then
		return new_base_point
	end
...
function collision.collide_and_slide(packet, gravity, lifter, slope_threshold)
	-- calculate position and velocity in eSpace
	local e_radius = packet.e_radius
	-- if lifter then
	-- 	packet.lifter = e_radius
	-- 	e_radius = lifter
	-- 	packet.e_radius = lifter
	-- end
	...
	-- Add gravity pull:
	if gravity and not lifter then
	...
	-- if lifter then
	-- 	packet.e_radius = packet.lifter
	-- 	packet.lifter = nil
	-- end

	packet.nearest_distance = packet.nearest_distance - packet.z_offset
